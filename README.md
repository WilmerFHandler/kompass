...
The report's `model` field is always the stable string `structural-v3`.
...
Kompass uses one additive structural score at function, file, and repository
...
and nesting inside the closure are scored in the closure's own report. Each
callable starts at depth zero. A file's `burden` is the sum of its callable
...
several moderate hotspots visible alongside an individual worst function.
...
aggregates those file measurements, and text output shows them next to
repository burden. A lower burden after a refactor is not a verified improvement if macro
...
forms contain different operation nodes. The report's `structural-v3` model
